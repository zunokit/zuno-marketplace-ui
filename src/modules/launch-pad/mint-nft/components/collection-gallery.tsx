...
export default function CollectionGallery({
  ...
  return (
    <div
      className={cn(
        " bg-linear-to-b from-secondary to-background dark:from-card dark:to-card rounded-os-xl relative overflow-hidden xl:max-w-2xl border border-border-subtle",
        hasThumbnails && "p-5"
      )}
    >
      <div className="absolute top-5 right-5 z-10">
        <Button
          variant="ghost"
          size="icon"
          className="bg-muted dark:bg-card border border-border-subtle text-foreground dark:text-foreground hover:bg-secondary dark:hover:bg-card/80 rounded-os-lg h-10 w-10 cursor-pointer"
      ...
      <div
        className="aspect-4/5 relative rounded-os-lg overflow-hidden"
        ...
        <div className="mt-5">
          <div className="flex gap-3 overflow-x-auto pb-2">
            {displayedThumbnails.map((img, index) => (
              <div
                key={index}
                className={`shrink-0 w-24 h-24 relative rounded-os-lg overflow-hidden cursor-pointer border-2 transition-all duration-200 hover:opacity-90 dark:hover:opacity-80 ${
                  index === currentIndex
